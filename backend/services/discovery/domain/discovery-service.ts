...
// Search cursor helpers — separate from snapshot cursor used in recommendations
type SearchIndexPath = 'countryGender' | 'religionGender' | 'all';
...
function encodeSearchCursor(path: SearchIndexPath, key: Record<string, unknown>): string {
  return Buffer.from(JSON.stringify({ path, key })).toString('base64');
}

function decodeSearchCursor(cursor: string): SearchCursor | undefined {
  try {
    const parsed = JSON.parse(Buffer.from(cursor, 'base64').toString());
    if (parsed.path && parsed.key) return parsed as SearchCursor;
    return undefined;
  } catch {
    return undefined;
  }
...
function getSearchIndexPath(filters: SearchFilters): SearchIndexPath {
  if (filters.country && filters.gender) return 'countryGender';
  if (filters.religion && filters.gender) return 'religionGender';
  return 'all';
}
...
export class DiscoveryService {
  ...
  async touchLastActive(userId: string): Promise<void> {
    ...
    // Only update if last active was more than 10 minutes ago
    // prevents hammering DynamoDB with writes on every single request
    const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000).toISOString();
    if (existing.lastActiveAt > tenMinutesAgo) return;
  ...
  async getRecentlyJoined(
    ...
    const candidates = filtered.slice(0, limit + 5);
    const activeMap = await this.discoveryRepo.getProfilesByIds(
  ...
  async search(
    ...
    // Reject inverted age range immediately
    if (
    ...
    const currentPath = getSearchIndexPath(filters);

    // Only use cursor if it came from the same index path
    let startKey: Record<string, unknown> | undefined;
    if (cursor) {
      const decoded = decodeSearchCursor(cursor);
      if (decoded && decoded.path === currentPath) {
        startKey = decoded.key;
      }
    }

    // Fail safe — if block list fetch fails, search still works
    let blockedIds = new Set<string>();
    try {
      const blockRecord = await this.coreRepo.get<{ blockedUserIds?: string[] }>(
        `USER#${userId}`,
        'BLOCK',
      );
      blockedIds = new Set<string>(blockRecord?.blockedUserIds || []);
    } catch (err) {
      logger.error('[DiscoveryService] Failed to load block list', { userId, error: String(err) });
    }

    const seen = new Set<string>();
    const items: DiscoveryProfile[] = [];
    const pageLimit = Math.min(50, Math.max(limit * 3, 30));
    const deadline = Date.now() + 20_000;
    let lastKey = startKey;
    let nextKey: Record<string, unknown> | undefined;

    // No maxPages limit — loop until:
    //   1. collected enough results (limit)
    //   2. DynamoDB has no more data (lastKey undefined)
    //   3. 20s deadline hit
    while (items.length < limit) {
      if (Date.now() > deadline) {
        logger.warn('[DiscoveryService] Search hit 20s deadline', { userId });
        break;
      }

      let results: { items: DiscoveryProfile[]; lastKey?: Record<string, unknown> };

      if (currentPath === 'countryGender') {
        results = await this.discoveryRepo.searchByCountryAndGender(
          filters.country!,
          filters.gender!,
          { limit: pageLimit, exclusiveStartKey: lastKey },
        );
      } else if (currentPath === 'religionGender') {
        results = await this.discoveryRepo.searchByReligionAndGender(
          filters.religion!,
          filters.gender!,
          { limit: pageLimit, exclusiveStartKey: lastKey },
      ...
      if (currentPath === 'all' && filtered.length > 0) {
        const activeMap = await this.discoveryRepo.getProfilesByIds(
      ...
      // DynamoDB has no more data — stop
      if (!results.lastKey) break;
    }

    const nextCursor = nextKey ? encodeSearchCursor(currentPath, nextKey) : undefined;
    return { items, nextCursor };
  }
}
